//! wkz notifications example.
//! Demonstrates UNUserNotificationCenter permission request and local
//! notification delivery from the JS bridge.
//!
//! Running:
//!   cd examples/notifications && zig build run
...
pub fn main() !void {
    ...
    defer app.deinit();
    try app.installDefaultMenu("wkz Notifications");

    const window = try wkz.window.Window.init(.{
        .width = 560,
        .height = 480,
        .title = "wkz Notifications Demo",
    });
    ...
    try wkz.notifications.registerHandlers(&bridge);
