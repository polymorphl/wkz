...
M8 — WebKit Inspector DX (dev-mode devtools + menu).

---

## M8 — WebKit Inspector DX

| ID | Task | Status |
|----|------|--------|
| 8.1 | `src/webview.zig` — `pub fn enableDevTools(self: WebView) void`: KVC `developerExtrasEnabled=true` sur `WKPreferences` (config→preferences→setValue:forKey:). NSNumber autoreleased, +1 NSString clé releasé via defer. | DONE |
| 8.2 | `src/menu.zig` — `standard_dev_menu: bool = false` dans `MenuBarConfig`. Bloc dans `buildMenuBar` après Window menu: NSMenu "Developer", item "Show Web Inspector", key `"i"`, modifier `(1<<19)\|(1<<20)` (Cmd+Opt). Action `selector: "showWebInspector:"`. Test: défaut false. | DONE |
| 8.3 | `examples/basic/src/main.zig` — remplacer `installDefaultMenu` par `setMenuBar(.{ .app=.{.name="basic"}, .standard_dev_menu=build_options.dev })`. Ajouter `if (build_options.dev) webview.enableDevTools();` après `webview.attach(window)`. | DONE |
...
| 6.4 | `src/events.zig` — window focus/blur via NSNotificationCenter + custom ObjC observer class; `examples/events/` | DONE |
